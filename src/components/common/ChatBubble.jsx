...
import Avatar from "@assets/icon/Avatar.jsx";
...
const ChatBubble = ({ type, contentType, message, nickName, image, profileImg, createdAt, showTime, showNickName, showAvatar, activeButton, onClickCheck }) => {
    ...
    <div className={clsx("flex flex-col", type === "USER" ? "items-end" : "items-start")}>
      <div className="flex items-start gap-2.5">
        {type !== "USER" &&
          <div className="w-8 h-8 rounded-full overflow-hidden">
            {showAvatar && (
              type === "OTHERS" ?
                profileImg ? <img src={profileImg} className="w-full h-full object-cover"/> : <Avatar/>
                :<FillyIcon className="w-full h-full"/>)}
        </div>}
        <div>
          {type === "OTHERS"  && showNickName && <p className="mb-1 text-014 font-normal">{nickName}</p>}
          <div
            className={clsx(
      ...
      {contentType === "image" && (
        <div className={clsx("flex mt-[8px]", type !== "user" ? "pl-10" : "")}>
          <ImageButton Icon={CheckIcon} text={t("button_yes")} isActive={activeButton === "yes"}
                       onClick={() => onClickCheck("yes")}/>
          <ImageButton Icon={CancelIcon} text={t("button_no")} isActive={activeButton === "no"}
                       onClick={() => onClickCheck("no")}/>
        </div>
      )}
